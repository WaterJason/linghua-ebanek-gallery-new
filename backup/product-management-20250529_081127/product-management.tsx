...
import { getProductUnits, getArtworkMaterials, addProductUnit, removeProductUnit, addProductMaterial, removeProductMaterial, batchUpdateArtworks } from "@/lib/actions/artwork-actions";
...
export function ProductManagement() {
  ...
  const [currentProduct, setCurrentProduct] = useState<Artwork | null>(null)
  const [currentCategory, setCurrentCategory] = useState<ArtworkCategory | null>(null)
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false)
  const [itemToDelete, setItemToDelete] = useState<Artwork | ArtworkCategory | null>(null)
  ...
  // 处理编辑产品
  const handleEditProduct = (product: Artwork) => {
    setCurrentProduct(product)
    setIsProductDialogOpen(true)
  }

  // 处理删除产品
  const handleDeleteProduct = (product: Artwork) => {
  ...
  // 处理编辑分类
  const handleEditCategory = (category: ArtworkCategory) => {
    setCurrentCategory(category)
    setIsCategoryDialogOpen(true)
  }

  // 处理删除分类
  const handleDeleteCategory = (category: ArtworkCategory) => {
    ...
    try {
      ...
      await executeBatchOperation(
        ...
        async (item, index) => {
          ...
          return await batchUpdateArtworks([item])
        },
        '批量编辑产品',
        'products',
        {
  ...
  const handleAddMaterial = async (material: string) => {
    try {
      ...
      // 重新加载材质数据
      const materialsData = await getArtworkMaterials();
      setMaterials(materialsData);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (deleteType === "product") {
        await enhancedExecuteOperation(
          async () => {
            return await deleteArtwork(itemToDelete.id!)
          },
          {
            playSound: true,
            soundType: 'warning',
    ...
    >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="analytics" className="space-y-4">
          ...
          <ProductAnalytics artworks={artworks} />
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={isProductDialogOpen} onOpenChange={setIsProductDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>{currentProduct ? "编辑作品" : "添加作品"}</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto pr-2">
            <ProductForm
              artwork={currentProduct}
              categories={categories}
              units={units}
              materials={materials}
            ...
            <div>
              ...
              {deleteType === "category" && (
                <div className="text-sm text-muted-foreground">
                  {(itemToDelete as ArtworkCategory).productCount} 个产品
                </div>
              )}
            </div>
          </div>
