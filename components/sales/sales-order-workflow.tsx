...
export function SalesOrderWorkflow({ order, onWorkflowUpdated }: SalesOrderWorkflowProps) {
  ...
  useEffect(() => {
    const loadWorkflowInstance = async () => {
      ...
      try {
        // 查询是否有与此销售订单关联的工作流实例
        const instance = await getWorkflowInstance("order", order.id.toString())
        if (instance) {
  ...
  useEffect(() => {
    const loadAvailableWorkflows = async () => {
      ...
      try {
        // 获取适用于销售订单的工作流
        const workflows = await getWorkflowByEntityType("order")
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base">审批流程</CardTitle>
        <CardDescription>此销售订单尚未启动审批流程</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoadingWorkflows ? (
          <div className="flex justify-center items-center py-6">
          ...
          <div className="text-center py-6 text-muted-foreground">
            ...
            <p>没有可用的审批流程</p>
            <p className="text-sm mt-1">请先在工作流管理中创建销售订单审批流程</p>
          </div>
        ) : (
          <div className="space-y-4">
      ...
      <Dialog open={showStartDialog} onOpenChange={setShowStartDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              为销售订单 #{order.orderNumber} 启动审批流程
            </DialogDescription>
          </DialogHeader>
