...
interface WorkshopAuditLogProps {
  workshopId: number
  className?: string
  showHeader?: boolean
  limit?: number
...
export function WorkshopAuditLog({
  workshopId,
  className,
  showHeader = true,
  limit = 5
}: WorkshopAuditLogProps) {
  const [isLoading, setIsLoading] = useState(false)
  const [isFullLogOpen, setIsFullLogOpen] = useState(false)

  // 查看完整日志
  const handleViewFullLog = () => {
    window.open(`/settings/audit-logs?entityType=workshop&entityId=${workshopId}`, "_blank")
  }
    ...
    <Card className={className}>
      {showHeader && (
        <CardHeader className="pb-3">
          ...
          <CardDescription>记录对此团建订单的所有操作</CardDescription>
        </CardHeader>
      )}
