...
interface ChannelAuditLogProps {
  channelId: number
  className?: string
  showHeader?: boolean
  limit?: number
...
export function ChannelAuditLog({
  ...
  showHeader = true,
  limit = 5
}: ChannelAuditLogProps) {
  const [isLoading, setIsLoading] = useState(false)
  const [isFullLogOpen, setIsFullLogOpen] = useState(false)

  // 查看完整日志
  const handleViewFullLog = () => {
    window.open(`/settings/audit-logs?entityType=channel&entityId=${channelId}`, "_blank")
  }
    ...
    <Card className={className}>
      {showHeader && (
        <CardHeader className="pb-3">
          ...
          <CardDescription>记录对此渠道的所有操作</CardDescription>
        </CardHeader>
      )}
