...
import { ProductDataAdapter } from "./data-adapter";
...
export class DataMigrationManager {
   ...
   * 检测产品数据的字段映射问题
   */
  static async detectFieldMappingIssues(): Promise<FieldMappingIssue[]> {
   ...
   * 修复废弃的category字段
   */
  static async fixDeprecatedCategoryField(): Promise<MigrationResult> {
    let processed = 0;
    let updated = 0;
    let errors = 0;
   ...
   * 标准化产品类型字段
   */
  static async standardizeProductTypes(): Promise<MigrationResult> {
    let processed = 0;
    let updated = 0;
    let errors = 0;
    const warnings: string[] = [];
   ...
   * 修复外键关联问题
   */
  static async fixForeignKeyIssues(): Promise<MigrationResult> {
    let processed = 0;
    let updated = 0;
    let errors = 0;
    const warnings: string[] = [];
   ...
   * 数据验证和清理
   */
  static async validateAndCleanData(): Promise<MigrationResult> {
    let processed = 0;
    let updated = 0;
    let errors = 0;
    const warnings: string[] = [];
  ...
  static async runFullMigration(): Promise<{
    ...
    try {
      ...
      const step1 = await this.fixDeprecatedCategoryField();
      results.push(step1);

      // 2. 标准化产品类型
      console.log("步骤2: 标准化产品类型");
      const step2 = await this.standardizeProductTypes();
      results.push(step2);

      // 3. 修复外键关联
      console.log("步骤3: 修复外键关联");
      const step3 = await this.fixForeignKeyIssues();
      results.push(step3);

      // 4. 数据验证和清理
      console.log("步骤4: 数据验证和清理");
      const step4 = await this.validateAndCleanData();
      results.push(step4);
