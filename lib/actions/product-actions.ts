...
export async function createProduct(data: CreateProductInput): Promise<PrismaProduct> {
  try {
    // 导入错误处理模块
    const { ValidationError, DatabaseError, AlreadyExistsError, handleError } = require('@/lib/error-handler');

    // 验证数据
    const validation = validateCreateProduct(data);
    if (!validation.isValid) {
      throw new ValidationError(validation.errors.join("; "), validation.errors, 'product-management');
    }
    ...
    if (existingProduct) {
      throw new AlreadyExistsError('产品名称、SKU或条形码已存在', {
        name: data.name,
        sku: data.sku,
        barcode: data.barcode
      }, 'product-management');
    ...
    // 使用统一的错误处理机制
    const { handleError, BusinessLogicError } = require('@/lib/error-handler');
    const appError = await handleError(error, 'product-management');
 ...
 * ```
 *
 * @throws 如果产品不存在、有库存记录、有销售记录或删除失败，会抛出错误
 *
 * @category 删除
 */
...
export async function deleteProductCategory(id: number): Promise<{ success: boolean }> {
  try {
    ...
    if (existingCategory._count.children > 0) {
      throw new Error("分类有子分类，无法删除");
    }

    // 检查分类是否有产品
    if (existingCategory._count.products > 0) {
      throw new Error("分类有产品，无法删除");
    }
